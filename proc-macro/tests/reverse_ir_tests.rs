...
{}", indent_json(&product_json_str));
    ...
    let struct_def = match &ir_back.types[0] {
        unistructgen_core::IRType::Struct(s) => s,
    ...
    // Check types
    assert!(matches!(price_field.ty, unistructgen_core::IRTypeRef::Primitive(unistructgen_core::PrimitiveKind::F64) | unistructgen_core::IRTypeRef::Primitive(unistructgen_core::PrimitiveKind::Decimal)));
    assert!(matches!(name_field.ty, unistructgen_core::IRTypeRef::Primitive(unistructgen_core::PrimitiveKind::String)));
}
