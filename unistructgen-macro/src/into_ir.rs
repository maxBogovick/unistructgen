...
pub fn impl_into_ir(input: TokenStream) -> TokenStream {
            ...
            match data.fields {
                Fields::Named(ref fields) => {
                    let recurse = fields.named.iter().map(|f| {
                        ...
                        quote! {
                            {
                                let ty_ref = <#field_type as unistructgen::core::IntoIR>::ir_type_ref();
                                ...
                                let constraints = unistructgen::core::ir::FieldConstraints {
                                    min_length: #min_len,
                                    max_length: #max_len,
                                    min_value: #min_val,
                                    max_value: #max_val,
                                ...
                                let mut field = unistructgen::core::ir::IRField::new(
    ...
    let definition_body = match input.data {
        Data::Struct(_) => {
             quote! {
                #fields_code
                let mut s = unistructgen::core::ir::IRStruct::new(#name_str.to_string());
                s.fields = fields;
                s.doc = #struct_doc;
                Some(unistructgen::core::ir::IRType::Struct(s))
            }
        }
        _ => quote! { None }
    };
    ...
    let expanded = quote! {
        impl unistructgen::core::IntoIR for #name {
            fn ir_type_ref() -> unistructgen::core::ir::IRTypeRef {
                unistructgen::core::ir::IRTypeRef::Named(#name_str.to_string())
            }

            fn ir_definition() -> Option<unistructgen::core::ir::IRType> {
                #definition_body
            }
        }
    };
