...
use unistructgen::parsers::env::{EnvParser, EnvParserOptions};
use unistructgen::parsers::sql::{SqlParser, SqlParserOptions};
use unistructgen::parsers::graphql::{GraphqlParser, GraphqlParserOptions};
...
pub fn generate_struct_from_json(input: TokenStream) -> TokenStream {
    ...
    if input.reverse_ir {
        for ty in &mut ir_module.types {
            if let unistructgen::core::IRType::Struct(s) = ty {
