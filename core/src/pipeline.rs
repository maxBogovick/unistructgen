...
use crate::{CodeGenerator, IRTransformer, Parser, TransformError};
...
///
/// ```ignore
/// use unistructgen_core::{Pipeline, IRTransformer};
    ...
    G: CodeGenerator,
{
    ...
    ///
    /// ```ignore
    /// use unistructgen_core::{Pipeline, transformer::{FieldOptionalizer, DocCommentAdder}};
    /// use unistructgen_json_parser::JsonParser;
    /// use unistructgen_codegen::RustRenderer;
    ///
    /// let parser = JsonParser::new(Default::default());
    /// let generator = RustRenderer::new(Default::default());
    ///
    /// let transformers: Vec<Box<dyn unistructgen_core::IRTransformer>> = vec![
